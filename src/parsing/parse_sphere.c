/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parse_sphere.c                                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: hlongin <hlongin@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/17 21:22:10 by hlongin           #+#    #+#             */
/*   Updated: 2026/01/15 14:11:05 by hlongin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "parsing.h"

static int	validate_sphere_tokens(char **tokens, t_vec3 *pos, double *diam,
		t_colour *color)
{
	if (!tokens[0] || !tokens[1] || !tokens[2] || !tokens[3])
		return (0);
	if (!parse_vec3(tokens[1], pos))
		return (0);
	if (!ft_atof_safe(tokens[2], diam) || *diam <= 0.0)
		return (0);
	if (!parse_color(tokens[3], color))
		return (0);
	return (1);
}

static void	fill_sphere(t_scene *scene, t_vec3 pos, double diam, t_colour color)
{
	int	idx;

	idx = scene->shapes.n_shapes;
	scene->shapes.shapes[idx].sphere.position = pos;
	scene->shapes.shapes[idx].sphere.radius = (float)(diam / 2.0);
	scene->shapes.colours[idx] = color;
	scene->shapes.sdfs[idx] = &sphere_sdf;
}

static void	fill_sphere_combine(t_scene *scene, t_combine_op *op)
{
	int	idx;

	idx = scene->shapes.n_shapes;
	scene->shapes.combine[idx] = op->func;
	scene->shapes.smoothing[idx] = op->smoothing;
	scene->shapes.n_shapes++;
}

int	parse_sphere(char *line, t_scene *scene, int line_num)
{
	char			**tokens;
	t_vec3			pos;
	double			diam;
	t_colour		color;
	t_combine_op	op;

	if (scene->shapes.n_shapes >= MAX_SHAPES)
		return (print_error(line_num, "Max shapes reached"), 0);
	tokens = ft_split(line, ' ');
	if (!tokens)
		return (0);
	if (!validate_sphere_tokens(tokens, &pos, &diam, &color))
		return (print_error(line_num, "Invalid sphere data"),
			free_split(tokens), 0);
	if (!parse_combine(tokens[4], &op))
		return (print_error(line_num, "Invalid combine op"), free_split(tokens),
			0);
	fill_sphere(scene, pos, diam, color);
	fill_sphere_combine(scene, &op);
	return (free_split(tokens), 1);
}
