...
/*   Updated: 2025/12/30 23:18:11 by hlongin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	fill_cylinder(t_scene *scene, t_vec3 pos, t_vec3 orient,
		double *dims)
{
	int	idx;
...
int	parse_cylinder(char *line, t_scene *scene, int line_num)
{
	...
	if (!parse_color(tokens[5], &color))
		return (printf("Error\nLine %d: Invalid cylinder data\n", line_num),
			free_split(tokens), 0);
	fill_cylinder(scene, pos, orient, dims);
	scene->shapes.colours[scene->shapes.n_shapes++] = color;
